...
data_path = Path(r'C:\Users\pinha\OneDrive\Documents\Trading\data') # set to e.g. external harddrive
...
def store_messages(m):
    ...
    with pd.HDFStore(itch_store) as store:
        for mtype, data in m.items():
            ...
            # apply alpha formatting if applicable
            if mtype in alpha_formats.keys():
                data = format_alpha(mtype, data)

            # Ensure consistent data types for string/object columns
            for col in data.select_dtypes(include=['object']).columns:
                data[col] = data[col].fillna('Unknown').astype(str)

            # Dynamically set min_itemsize only for object columns
            min_itemsize = {col: 10 for col in data.select_dtypes(include=['object']).columns}

            # Dynamically set data_columns based on actual columns
